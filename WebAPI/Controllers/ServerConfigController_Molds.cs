...
namespace iChen.Web
{
	public partial class ServerConfigController
	{
		...
		{
			if (!Sessions.IsAuthorized(Request, out var orgId, Filters.Mold)) return Unauthorized();

			// TODO: GroupBy doesn't seem to work in async - fix when ASP.NET Core supports it
			var mx3 = await db.Molds.AsNoTracking()
											.Where(m => m.Controller.OrgId.Equals(orgId, StringComparison.OrdinalIgnoreCase))
											.ToListAsync(ct)
											.ConfigureAwait(false);

			var mx = mx3.GroupBy(m => m.Name)
									.ToDictionary(g => g.Key, g => g.ToDictionary(x => x.ControllerId.GetValueOrDefault(0), x => new MoldX(x)) as IReadOnlyDictionary<int, MoldX>);
